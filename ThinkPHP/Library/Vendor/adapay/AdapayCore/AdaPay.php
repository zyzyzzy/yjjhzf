...
class AdaPay
{
    ...
    public static function init($config_info, $prod_mode="live", $is_object=false){
        ...
        self::$gateWayUrl =  defined("GATE_WAY_URL") ? GATE_WAY_URL : "https://api.adapay.tech";
    ...
    public function isNotifyurl(){
        if (empty( $this->result )){
            return true;
        }

        $resp_sign = $this->result['sign'];
        $resp_data = json_decode($this->result['data'],true);
        file_put_contents('huifu1.txt',$resp_sign."\n",FILE_APPEND);
        if(!$resp_data || !$resp_sign){
            file_put_contents('huifu2.txt',json_encode($_POST)."\n",FILE_APPEND);
            return true;
        }else{
            if ($this->ada_tools->verifySignNotifyurl($resp_sign, $this->result['data'])){
                if($this->result['type'] == 'payment.success' && $resp_data['status'] == 'succeeded'){
                    $this->result = $resp_data;
                    file_put_contents('huifu4.txt',json_encode($_POST)."\n",FILE_APPEND);
                    return false;
                }
            }else{
                file_put_contents('huifu3.txt',$resp_sign.'\n'.json_encode($resp_data)."\n",FILE_APPEND);
                return true;
            }
        }
    }
