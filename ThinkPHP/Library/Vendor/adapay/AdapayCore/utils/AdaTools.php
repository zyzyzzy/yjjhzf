...
class AdaTools
{
    ...
    public function verifySign($signature, $data){
        if($this->checkEmpty($this->rsaPublicKeyFilePath)){
           // file_put_contents('huifu5.txt',$this->rsaPublicKey."\n",FILE_APPEND);
            $pubKey=$this->rsaPublicKey;
            $key = "-----BEGIN PUBLIC KEY-----\n".wordwrap($pubKey, 64, "\n", true)."\n-----END PUBLIC KEY-----";
        }else {
           // file_put_contents('huifu6.txt',$this->rsaPublicKeyFilePath."\n",FILE_APPEND);
            $pubKey = file_get_contents($this->rsaPublicKeyFilePath);
            $key = openssl_get_publickey($pubKey);
        }
       // file_put_contents('huifu8888.txt',json_encode($data)."\n",FILE_APPEND);
    ...
    public function verifySignNotifyurl($signature, $data){
        if($this->checkEmpty($this->rsaPublicKeyFilePath)){
            file_put_contents('huifu5.txt',$this->rsaPublicKey."\n",FILE_APPEND);
            $pubKey=$this->rsaPublicKey;
            $key = "-----BEGIN PUBLIC KEY-----\n".wordwrap($pubKey, 64, "\n", true)."\n-----END PUBLIC KEY-----";
        ...
        file_put_contents('huifu8888.txt',$data."\n",FILE_APPEND);
        if (openssl_verify($data, base64_decode($signature), $key)){
            return true;
        }else{
            return false;
