...
class IndexController extends SettleController {
    ...
    public function index()
    {
        ...
        if (!(A("Version/IndexDaifu")->index(I("request."), $this->returnJson))) {
           ...
           // exit(json_encode($this->returnJson,JSON_UNESCAPED_UNICODE));  //输出JSON格式的错误信息
          parent::indexsettle($this->returnJson);
        }
    ...
    {
        $switch = M('website')->where('id=1')->field('settle_valve,all_valve')->find();
        //0:开启 1:关闭
        if ($switch['all_valve']==1 || $switch['settle_valve']==0) {
