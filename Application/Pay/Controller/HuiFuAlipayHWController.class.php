...
class HuiFuAlipayHWController extends PayController
{
    protected $PayName = "HuiFuAlipayHW";  //通道编码
...
# 对支付结果进行处理
        if ($payment->isError()){
            ...
            if(I('qrcode') == 'url'){
                $this->ajaxReturn([
                    'qrcode' => $payment->result['expend']['pay_info']
                ], 'json');
            }else{

                if(parent::isMobile()){
//                    $alipayurl = 'alipayqr://platformapi/startapp?saId=10000007&clientVersion=3.7.0.0718&qrcode='.$payment->result['expend']['pay_info'];
//                    header("Location:".$alipayurl);
                    header("Location:".$payment->result['expend']['pay_info']);
                }else{
                    parent::Qrcode($payment->result['expend']['pay_info'],$parameter["sysordernumber"],$parameter["userordernumber"],$this->PayName,$parameter["ordermoney"]);
                }
