...
class UserauthgroupaccessModel extends Model
{
    ...
    public static function delcid($id, $user_id)
    {
        ...
        M('userauthgroupaccess')->where($where)->delete();
    }


    /**
     * 2019-1-23 任梦龙：通过主用户id和角色id删除所有用到该角色的子账号
     * @param $user_id
     * @param $group_id：角色id
     */
    ...
    public static function findUserGroupAccess($id, $user_id)
    {
        ...
        return M('userauthgroupaccess')->where($where)->getField('group_id', true);
    }
    ...
    public static function getUserRules($user_id, $child_id)
    {
        ...
        $where = array(
            'a.cid' => $child_id,
            'a.user_id' => $user_id,  //2019-1-18 任梦龙：添加用户id的条件
            'b.status' => 1,
            'b.del' => 0,
        );
    ...
    public static function getAllUserMenu()
    {
        ...
        $all_menus = M('userauthrule')->where($where)->select();
    ...
    {
        return M('userauthgroupaccess')->where(['group_id' => $group_id, 'user_id' => $user_id])->field('cid')->select();
    }
