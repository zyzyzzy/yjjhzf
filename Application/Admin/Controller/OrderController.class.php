...
class OrderController extends CommonController
{
    ...
    public function orderList()
    {
        ...
        $shangjias = M('payapishangjia')->where('del = 0')->select();

        $this->assign('shangjias', $shangjias);



        $payapis = M('payapi')->where('del = 0')->field('id,zh_payname')->select();

        $this->assign('payapis', $payapis);



        $accounts = M('payapiaccount')->where('del = 0')->select();
    ...
    public function loadOrderList()
    {
        ...
        //SELECT * FROM `pay_orderinfo` WHERE unix_timestamp( datetime ) between unix_timestamp( '2019-07-28 13:39:00 ') and unix_timestamp( '2019-07-28 13:39:30' ) ORDER BY `orderid` desc LIMIT 0,10
...
//            $where[$i] = "unix_timestamp( datetime ) between unix_timestamp( '{$success_start} ') and unix_timestamp( '{$success_end}' )";
...
//        }
        ...
        $redis_h_name = "amnpay:orderinfo:where".md5(serialize($where1));

        if (!$redis->hgetall($redis_h_name)) {
            $redis->hset($redis_h_name, 'true_ordermoney', M('orderinfo')->where($where1)->sum('true_ordermoney'));

            $redis->hset($redis_h_name, 'moneycost', M('orderinfo')->where($where1)->sum('moneycost'));

            $redis->hset($redis_h_name, 'moneytrade', M('orderinfo')->where($where1)->sum('moneytrade'));

            $redis->hset($redis_h_name, 'money', M('orderinfo')->where($where1)->sum('money'));

            $redis->hset($redis_h_name, 'freezemoney', M('orderinfo')->where($where1)->sum('freezemoney'));

            $redis->hset($redis_h_name, 'count_success', M('orderinfo')->where($where1)->count());

            $redis->expire($redis_h_name, 120);
        }



        $sum_ordermoney = $redis->hget($redis_h_name, 'true_ordermoney');

        $sum_costmoney = $redis->hget($redis_h_name, 'moneycost');//成本金额

        $sum_trademoney = $redis->hget($redis_h_name, 'moneytrade');//手续费

        $sum_money = $redis->hget($redis_h_name, 'money');//到账金额

        $sum_freezemoney = $redis->hget($redis_h_name, 'freezemoney');//冻结金额

        $count_success = $redis->hget($redis_h_name, 'count_success');//成功笔数
        ...
        $redis_h_name = "amnpay:orderinfo:where:fail".md5(serialize($where2));

        if (!$count_fail = $redis->get($redis_h_name)) {
            $count_fail = M('orderinfo')->where($where2)->count();//失败笔数

            $redis->set($redis_h_name, $count_fail);
        ...
        $redis_h_name = "amnpay:orderinfo:where:fail:test".md5(serialize($where3));

        if (!$count_test = $redis->get($redis_h_name)) {
            $count_test = M('orderinfo')->where($where3)->count();//测试笔数

            $redis->set($redis_h_name, $count_test);
        ...
        foreach ($datalist as $k => $v) {
            if (!$datalist[$k]['payapiname'] = $redis->get('amnpay:payapi:zh_payname:'.$v['payapiid'])) {
                $zh_payname = $this->getNameById('payapi', $v['payapiid'], 'zh_payname');

                $redis->set('amnpay:payapi:zh_payname:'.$v['payapiid'], $zh_payname);

                $datalist[$k]['payapiname'] = $zh_payname;
            }
...
//
            ...
            if (!$datalist[$k]['payapiclassname'] = $redis->get('amnpay:payapiclass:classname:'.$v['payapiclassid'])) {
                $classname = M('payapiclass')->where('id=' . $v['payapiclassid'])->getField('classname');

                $redis->set('amnpay:payapiclass:classname:'.$v['payapiclassid'], $classname);

                $datalist[$k]['payapiclassname'] = $classname;
            }



            if (!$datalist[$k]['statusname'] = $redis->get('amnpay:orderinfo:statusname:'.$v['status'])) {
                $statusname = $statustype[$v['status']];

                $redis->set('amnpay:orderinfo:statusname:'.$v['status'], $statusname);

                $datalist[$k]['statusname'] = $statusname;
            }
    ...
    public function changeType()
    {
        ...
        if ($res) {
            ...
            $this->ajaxReturn(['status' => 'ok', 'msg' => '修改失败']);
        }
    }
    ...
    public function downloadOrderList()
    {
        ...
        $sysordernumber = I("get.sysordernumber", "");
        ...
        $menu_zh = array('商户号', '用户订单号', '提交金额', '实际金额', '冻结金额', '到账金额', '通道分类', '通道名称', '订单时间', '成功时间', '状态', '投诉状态', '类型');

        $menu_en = array('memberid', 'userordernumber', 'ordermoney', 'true_ordermoney', 'freezemoney', 'money', 'payapiclassname', 'payapiname', 'datetime', 'successtime', 'status', 'complaint', 'type');

        $config = array('RowHeight' => 25, 'Width' => 20);

        $this->addAdminOperate('管理员[' . session('admin_info.user_name') . ']导出了交易记录列表');
    ...
    public function loadOrderLog()
    {
        ...
        $sys_order_num = I("post.sys_order_num", "", 'trim');

        if ($sys_order_num <> "") {
            $where[$i] = "(sys_order_num ='" . $sys_order_num . "')";

            $i++;
        }

        $user_order_num = I("post.user_order_num", "", 'trim');

        if ($user_order_num <> "") {
            $where[$i] = "(user_order_num ='" . $user_order_num . "')";
        ...
        if ($start <> "") {
            $where[$i] = "DATEDIFF('" . $start . "',at_time) <= 0";
        ...
        if ($end <> "") {
            $where[$i] = "DATEDIFF('" . $end . "',at_time) >= 0";
        ...
        $count = M('orderlog')->alias('a')->where($where)->join('LEFT JOIN __USER__ b ON a.user_id = b.id')->count();

        $page = I('post.page');

        $limit = I('post.limit');

        $datalist = M('orderlog')->alias('a')->where($where)->order('at_time desc')->page($page, $limit)

            ->join('LEFT JOIN __USER__ b ON a.user_id = b.id')->field('a.*,b.username')->select();
    ...
    public function downloadChangeList()
    {
        ...
        $sys_order_num = I("get.sys_order_num", "", 'trim');
        ...
        $user_order_num = I("get.user_order_num", "", 'trim');
        ...
        $datalist = M('orderlog')->alias('a')->where($where)->order('at_time desc')
        ...
        $menu_zh = array('系统订单号', '用户订单号', '用户名', '日志内容', '记录时间');

        $menu_en = array('sys_order_num', 'user_order_num', 'username', 'content_log', 'at_time');

        $config = array('RowHeight' => 25, 'Width' => 20);

        $this->addAdminOperate('管理员[' . session('admin_info.user_name') . ']导出了交易订单日志表');
