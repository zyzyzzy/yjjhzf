...
class VersionController extends Controller
{
    ...
    final public function index($parameter, &$returnJson, $obj)
    {
        ...
        foreach ($this->parameterarray as $key => $value) {
            ...
            if($value and (!array_key_exists($key,$parameter) or $parameter[$key] == "")){
        ...
        //2019-08-24 张杨  签名参数数组可以为空，如果为空时，参与签名的参数为所有非空的参数
        if (!(property_exists($obj, 'encryptedFields')) || !(is_array($this->encryptedFields))) {
          //  $this->returnJson["msg"] = "该版本中没有定义签名需要的字段";
          //  return false;
            $this->encryptedFields = [];
            foreach ($this->parameterarray as $key => $value){
                if(!array_key_exists($key,$parameter) or $parameter[$key] == ""){
                    continue;
                }else{
                    array_push($this->encryptedFields,$key);
                }
            }
     ...
     * 
     */
    final public function CheckOrderid($memberid, $orderid,$tablename="order")
    {
        if (M($tablename)->lock(true)->where("userordernumber='" . $orderid . "' and memberid = '" . $memberid . "'")->count() > 0) {
            return false;
        } else {
            return true;
        }
    ...
    final public function checkUserVersion($user_id, $version, $versiontype = 'pay')
    {
        ...
        //2019-3-28 任梦龙：修改字段名称
        $user_version_id = M($version_arr['tablename'])->where("numberstr='" . $version . "'")->getField("id");
        if ($user_version) {
            //用户指定了版本,只判断是否在指定版本中
        ...
        //2019-4-28 rml：修改
        //用户未指定版本,判断是否在通用版本中
        $public_version_arr = M($version_arr['tablename'])->where(['status' => 1, 'all_use' => 1, 'del' => 0])->getField('id', true);
        if ($public_version_arr) {
            //通用版本存在
            if (!in_array($user_version_id, $public_version_arr)) {
                $res['status'] = false;
                $res['msg'] = 'version 参数错误,不在通用版本中';
                return $res;
            }
        } else {
            //通用版本不存在
        ...
        //2019-4-28 rml：判断版本的状态在入口处已经判断过，所以这里就不必了
        //201-04-09汪桂芳添加:判断版本状态
//        $version_status = M($version_arr['tablename'])->where("id=".$user_version_id)->field("status,del")->find();
//        if($version_status['status']==2 || $version_status['del']==1){
//            //通用版本不存在
//            $res['status'] = false;
//            $res['msg'] = 'version 参数错误,该版本已禁用或已被删除';
//            return $res;
//        }else{
//            $res['status'] = true;
//            $res['msg'] = '';
